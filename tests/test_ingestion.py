...
def test_has_attachments_follows_the_attachments() -> None:
    assert msg().has_attachments is False
    assert msg(attachments=(Attachment("aneks.pdf"),)).has_attachments is True


def test_names_are_derived_from_the_attachments_not_stored_twice() -> None:
    message = msg(attachments=(Attachment("zgoda.pdf"), Attachment("cennik.xlsx")))

    assert message.attachment_names == ("zgoda.pdf", "cennik.xlsx")
...
class FakeAttachmentData:
    """The PropertyAccessor on an attachment, which can refuse like the real one."""
...
class FakeAttachment:
    def __init__(self, filename: str, data: object = b"", size: int | None = None) -> None:
        self.FileName = filename
        self.Size = size if size is not None else (len(data) if isinstance(data, bytes) else 0)
        self.PropertyAccessor = FakeAttachmentData(data)


class FakeAttachments:
    """Outlook's 1-indexed attachment collection."""

    def __init__(self, *items: FakeAttachment) -> None:
        self._items = items
...
def test_bytes_are_pulled_for_a_readable_attachment() -> None:
    from arbitrium.ingestion.outlook_mapi import _attachments

    item = FakeComItem(Attachments=FakeAttachments(FakeAttachment("zgoda.pdf", b"%PDF-1.4")))

    assert _attachments(item) == (Attachment("zgoda.pdf", b"%PDF-1.4", 8),)


def test_a_signature_logo_is_listed_but_its_bytes_are_never_moved() -> None:
    from arbitrium.ingestion.outlook_mapi import _attachments

    item = FakeComItem(Attachments=FakeAttachments(FakeAttachment("logo.png", b"PNGDATA")))
    (logo,) = _attachments(item)

    assert logo.filename == "logo.png"
    assert logo.data is None
    assert logo.size_bytes == 7


def test_an_oversized_file_is_listed_without_being_read() -> None:
    from arbitrium.attachments.base import MAX_ATTACHMENT_BYTES
    from arbitrium.ingestion.outlook_mapi import _attachments

    huge = FakeAttachment("katalog.pdf", b"%PDF", size=MAX_ATTACHMENT_BYTES + 1)
    (result,) = _attachments(FakeComItem(Attachments=FakeAttachments(huge)))

    assert result.data is None


def test_an_attachment_outlook_refuses_to_hand_over_is_still_listed() -> None:
    # Embedded messages and OLE objects raise on the data property. The file
    # must still show up, or a run would silently under-report what arrived.
    from arbitrium.ingestion.outlook_mapi import _attachments

    refused = FakeAttachment("zgoda.pdf", RuntimeError("com error"))
    (result,) = _attachments(FakeComItem(Attachments=FakeAttachments(refused)))

    assert result.filename == "zgoda.pdf"
    assert result.data is None


def test_an_unnamed_attachment_is_dropped() -> None:
    from arbitrium.ingestion.outlook_mapi import _attachments

    item = FakeComItem(Attachments=FakeAttachments(FakeAttachment("", b"x")))

    assert _attachments(item) == ()


def test_an_item_that_refuses_its_attachments_yields_none_rather_than_failing() -> None:
    from arbitrium.ingestion.outlook_mapi import _attachments

    assert _attachments(FakeComItem()) == ()
